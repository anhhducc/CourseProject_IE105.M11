<?php
    ...
    if(isset($_REQUEST['signup'])) {
        ...
        $password2 = mysqli_real_escape_string($root_conn, $_REQUEST['password2']);

        if ($password === $password2) {
            $sql = "INSERT INTO Users(username, password) VALUES ('$username', '$password')";
            mysqli_query($root_conn, $sql);
            header("Location: index.php");
            exit();
        }
        else {
            echo "<h3 class='container bg-dark p-3 text-center text-warning rounded-lg mt-5'>Failed to sign up<h3>";
        }
    }
